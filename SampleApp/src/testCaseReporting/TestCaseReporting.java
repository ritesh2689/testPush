...
public class TestCaseReporting extends Reporting {
	...
	public void Openfile() {
		try {
			...
			// "/TestCase/" +
			// testCaseName+" ("+GlobalVar.CURRENT_EXECUTION_MODE+")"+".html",
			// true);
			...
			htmlfile = new FileOutputStream(SuiteReporting.pathToSuiteFolder
					...
					+ GlobalVar.BrowserName + ".html", true);
	...
	public void teststepreporting(String strActualResult, String strPassFail,
			String sExpectedMessage) { // ##U
		try {
			if (!GlobalVar.Reporting)
				return;
			...
			try {
				if (GlobalVar.CURRENT_EXECUTION_MODE.equalsIgnoreCase("API")) {
					...
					if (GlobalVar.screenshotOption.equals("Fail")
							&& strPassFail == "FAIL") {
						nameOfScreenShot = captureImage();
						imgLink = "<a href=\"" + "../screenShot/"
								+ nameOfScreenShot + "\">Snapshot</a>";
					} else if (GlobalVar.screenshotOption.equals("ALL")) {
						nameOfScreenShot = captureImage();
						imgLink = "<a href=\"" + "../screenShot/"
								+ nameOfScreenShot + "\">Snapshot</a>";
					}
	...
	public void header() {
		try {
			...
			printhtml.println("<h2 align='center'>" + testCaseName
					+ " Test Case - ShopClues</h2>");
			printhtml
			...
			if (GlobalVar.detailReporting == true) {
				printhtml
						.println("<td width='28%' bgcolor='#CCCCFF' align='center'><b><font color='#000000' face='Tahoma' size='2'>Time of Screenshot</font></b></td>");
			}
			printhtml.println("</tr>");
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public void footer() { // ##U
		try {
			...
			GlobalVar.LastTestCaseSuccess = String.valueOf(SuccessRate);
