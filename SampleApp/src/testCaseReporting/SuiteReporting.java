...
public class SuiteReporting extends Reporting {
	...
	public void checkTestCaseStatus(File testCaseFile) {
		...
		try {
			...
			addTestToConsolidate(Status,
					getTestCaseName(testCaseFile.getName()), resultLink,
					sPercent);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public File createTempDir() throws IOException {
		...
		do {
			...
			String dirName = "ShopClues_Results_" + GlobalVar.serverIP + "_"
					+ getTime();
	...
	public void createResultFolderStructure() {
		try {
			...
			pathToSuiteFolder = createDir(PathToConsolidateResult,
					suiteName + sStartTimeConsolidate).getAbsolutePath();
	...
	public void copyFolder(File srcFolderPath, File targertFolderPath) {
		try {
			FileUtils
					.copyDirectoryToDirectory(srcFolderPath, targertFolderPath);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
